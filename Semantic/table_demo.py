...
class Semantic_:
    ...
    def read_token_file(self, all_word):
        with open(all_word, 'r', encoding='UTF-8')as f:
            lines = f.readlines()
        ...
        while index < len(self.token_list) - 2:
            ...
            if self.token_list[index].val == '=':  # a = b + c 判断 变量是否定义、是否是同类型变量
                ...
                while self.token_list[index].val != ';':  # 一个赋值表达式
                    if self.token_list[index + 2].val == '(':  # 表示函数调用参数匹配与否
    ...
    def access(self, all_word):
        self.read_token_file(all_word)
        self.Traver()
        self.syn_table.showTheInfo()
        self.var_list = self.syn_table.var_list
        for i in self.syn_table.var_list:
            print(str(type(i)).split('\'')[1].split('.')[-1])
