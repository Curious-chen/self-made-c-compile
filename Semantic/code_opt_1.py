...
class Optimizathon_:
    ...
    def DAG(self, fun_, AAA):
        ...
        for index__, __block in enumerate(self.basic_block):  # 对每个基本块进行优化
            ...
            self.node_dic.clear()
            self.node_dic.append(copy.deepcopy(self.nodes))
            for i in self.nodes:
                            ...
                            if len(node.value) != 0:
                                for per_ in node.value:
                                    if len(per_) > 1 and per_[1] > _node[1]:
                                        per_[1] -= 1
                        self.n -= 1
            for index_, node in enumerate(self.nodes):  # 删除每个节点多余的等价节点
                if len(node.value) > 1:
                    for per in node.value[1:]:
                        node.value.remove(per)
                        for node in self.nodes:
                            if len(node.value) != 0:
                                for per_ in node.value:
                                    if len(per_) > 1 and per_[1] > per[1]:
                                        per_[1] -= 1
                        self.n -= 1
        else:
            node_list = []
                ...
                if len(node.value) != 0:
                    for index, key in enumerate(node.value):
                        if key[1] == self.count and node.leftchild == None and node.rightchild == None:  # 赋值表达式
                            if not self.is_number(node.val):
    ...
    def get_tree(self):
        ...
        path = 'chensong/cache/DAG_tree.png'
