...
class Genration_:
    def __init__(self):
        ...
        self.token_list, self.sen_list, self.FC, self.lev_stake, self.or_TC_list, self.jump_list, self.break_list, self.contnue_list = [], [], [], [], [], [], [], []
    ...
    def read_token_file(self):
        ...
        with open(path, 'r', encoding='UTF-8')as f:
            ...
            for index, pre_line in enumerate(all_word):
                ...
                self.token_list.append(self.Token(pre_line[1], str(pre_line[2]), pre_line[0], index))
    ...
    def func(self, token):
        if token[self.index].val in ['void', 'int', 'char', 'float', 'double'] and token[
            self.index + 1].typ == '700':  # 函数定义生成四元式
