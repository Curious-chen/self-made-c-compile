from Demo import *
from table import *
from code.config import save_quaternion_info
...
'''
    ...
    _data = ['.data', 'RETURN dword ?', "stop byte 'pause', 0", "printf byte '%d ' ,0", "scanf byte '%d',0"]
    # ;.const 里面存放常量
    _const = ['.const']
    # ;.code是代码段。
    _code = ['.code']
    ...
    def __init__(self,token_list=list()):
        ...
        self.code_.Acess_(save_quaternion_info)
        self.qua_list = self.code_.block_dic
        self.Se = Semantic_()
        if token_list:
            print(token_list)
            self.Se.access_(token_list)
        else:
            self.Se.access()
