...
class SecTable:
    ...
    def checkHasDefine(self, token, lev):  # 重复定义语义错误
        if token[2] in list(self.variableDict.keys()):
            ...
            ErrorType = '重复定义'
            Location = 'Location:line {line} col {col}'.format(line=token.cur_line + 1, col=token.cur_col)
    ...
    def checkSameType(self, list_, globalVar, fun_list):  # 判断表达式的类型是否相同
        ...
        for ind in range(0, len(list_)):
            ...
            if type_ != type__:
                ErrorType = '参数类型不同'
                Location = 'Location:line {line} col {col}'.format(line=temp.cur_line + 1, col=temp.cur_col)
        ...
        while temp_token.val != ';':
            if temp_token.typ in ['700', '400', '500']:  # 表示标识符
                ...
                if type_ != type__:  # 如果函数类型与返回值类型不匹配则报错
                    ErrorType = '返回值类型不同'
                    Location = 'Location:line {line} col {col}'.format(line=temp_token.cur_line + 1,
                                                                       col=temp_token.cur_col)
...
class SYMBOL:
    ...
    def checkFunction(self, RES_TOKEN, id):  # 调用函数时候检查函数是否定义
        if RES_TOKEN[id].val not in self.globalNameList:
            ErrorType = '未定义的函数'
            Location = 'Location:line {line} col {col}'.format(line=RES_TOKEN[id].cur_line + 1,
                                                               col=RES_TOKEN[id].cur_col)
