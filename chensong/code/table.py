from chensong.code.Symbol_table import *
...
class Semantic_:
    ...
    def read_token_file(self, path=''):
        path = path if path else '../chen_si_yu/token_list.txt'
        with open('../cache/token_list.txt', 'r', encoding='UTF-8') as f:
        ...
        while index < len(self.token_list) - 2:
            ...
            if self.token_list[index].val == '=':  # a = b + c 判断 变量是否定义、是否是同类型变量
                ...
                if self.token_list[index - 1].typ in ['400', '800']:
                    ErrorType = '赋值错误'
                    Location = 'Location:line {line} col {col}'.format(line=self.token_list[index - 1].cur_line + 1,
                ...
                while self.token_list[index].val != ';':  # 一个赋值表达式
                    if self.token_list[index + 1].typ == '700' and self.token_list[
                        index + 2].val == '(':  # 表示函数调用参数匹配与否#TODO
            ...
            if self.token_list[index].val in ['break', 'continue', 'return']:
                ...
                if self.token_list[index].val in ['break', 'continue'] and tmp in ['if', 'else'] and len(
                    ...
                    ErrorType = '关键字使用错误'
                    Location = 'Location:line {line} col {col}'.format(line=self.token_list[index].cur_line + 1,
